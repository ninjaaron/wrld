...
not the case. The world sucks (well, for many people. My life actually
doesn't suck at all). What doesn't suck is iterating on stdin. Probably
my favorite thing to do. In the shell, the sanest way to do this is with
a ``while read line; do`` loop. Forget the world. ``wrld`` is the
future of iteration. (eh... yeah.)
...
With ``wrld``, you can write like this: ``fine -name '*foo.bar' -type f
| wrld mv {} '@sed "s/pat/rep/"'``. You can do somehting similar with
